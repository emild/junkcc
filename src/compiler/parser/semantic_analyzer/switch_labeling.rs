        ...
        UnlabeledStatement::Break(break_type, break_label) => {
            match break_type {
                Some(BreakType::Switch) => {
                    ...
                    if switch_stmnt_info.is_none() {
                        return Err(format!("break statement outside loop or switch"));
                    }

                    break_label.replace(switch_stmnt_info.as_ref().unwrap().switch_label.clone());
                },
                _ => {}
            }
        },
        UnlabeledStatement::Compound(block) => {
            label_block_switch_statements(block, switch_stmnt_info, case_labels_map, default_label)?;
        },
        UnlabeledStatement::Switch(expr, body, switch_stmnt_label, switch_stmnt_case_label_map, switch_stmnt_default_label) => {
            let switch_stmnt_global_label = make_unique_global_switch_label();
            switch_stmnt_label.replace(switch_stmnt_global_label.clone());
            label_statement_switch_statements(
                body,
                &Some(SwitchStatementInfo {
                    switch_label: switch_stmnt_global_label.clone(),
                    expr_type: typex_get_type(expr)
                }),
                switch_stmnt_case_label_map,
                switch_stmnt_default_label
            )?;
        },
        UnlabeledStatement::DoWhile(body, _, _) |
        UnlabeledStatement::While(_, body, _) |
        UnlabeledStatement::For(_, _, _, body, _) => {
            label_statement_switch_statements(body, switch_stmnt_info, case_labels_map, default_label)?;
        },
        UnlabeledStatement::If(_, then_stmnt, else_stmnt) => {
            label_statement_switch_statements(then_stmnt, switch_stmnt_info, case_labels_map, default_label)?;
            if let Some(else_stmnt) = else_stmnt {
                label_statement_switch_statements(else_stmnt, switch_stmnt_info, case_labels_map, default_label)?;
            }
        },
...
fn label_statement_switch_statements(stmnt: &mut Statement, switch_stmnt_info: &Option<SwitchStatementInfo>, case_labels_map: &mut HashMap<Const, String>, default_label: &mut Option<String>) -> Result<(), String>
{
    match stmnt {
        Statement::Stmnt(None, unlabeled_stmnt) => {
            label_unlabeled_statement_switch_statements(unlabeled_stmnt, switch_stmnt_info, case_labels_map, default_label)?;
        },
            ...
            for label in labels {
                match label {
                    Label::Case(expr) => {
                        ...
                        let global_case_label = make_unique_case_label(&switch_stmnt_info.switch_label, case_value.to_i64());
                    ...
                    Label::Default => {
                        ...
                        let global_default_label = make_global_default_label(&switch_stmnt_info.switch_label.clone());
...
fn label_block_item_switch_statements(block_item: &mut BlockItem, switch_stmnt_info: &Option<SwitchStatementInfo>, case_labels_map: &mut HashMap<Const, String>, default_label: &mut Option<String>) -> Result<(), String>
{
    match block_item {
        ...
        BlockItem::S(stmnt) => {
            label_statement_switch_statements(stmnt, switch_stmnt_info, case_labels_map, default_label)?;
            Ok(())
        }
    }
}
     ...
     match block {
        Block::Blk(block_items) => {
            for block_item in block_items {
                label_block_item_switch_statements(block_item, switch_stmnt_info, case_labels_map, default_label)?;
            }
        }
    };
